...
**Known issue, not yet fixed:** Nemotron occasionally emits a literal `<tool_call>`/`</tool_call>` text token inside a `delta.content` chunk itself — alongside, not instead of, a proper native `delta.tool_calls` entry — and `streamChat` streams `delta.content` straight to `onText` with no filtering, so the stray tag can reach the debater-visible reply. Reproduced while testing the `web_search` tool (2026-09), but it's a `streamChat`/model-output parsing gap unrelated to that tool specifically — any tool call could trigger it. See the comment at the `delta.content` handling in `aiClient.mjs`. Needs its own investigation (likely whether it's a vLLM/NIM chat-template quirk worth stripping before `onText`) rather than a fix bundled into an unrelated task.

**Corpus-privacy tools follow a two-part contract, not just a schema:** a tool like `search_corpus` or `web_search` being silent isn't enforced by code — the tool's own `description` plus a matching system-prompt section (`buildRetrievalCorpusSection()` / `WEB_SEARCH_SECTION` in `prompt.mjs`) are what tell the model never to narrate retrieval or name a source. When wiring a new silent tool, both halves need to agree; a tool whose description says "cite this openly" while chat expects silence (as `web_search` did before its 2026-09 rewrite) will leak the source with no code path to catch it.
