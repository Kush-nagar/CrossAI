...
- **Use debate-native language, not academic formalization.** Explain warrants the way a strong PF coach actually talks, not the way a philosophy paper does. Do not dress up an ordinary warrant in mathematical notation, probability expressions ("P(X) > 0"), or academic jargon ("epistemically void," "causal efficacy," "truth-tracking function") — say the same thing in plain strategic language instead. Concretely: say "the ballot can fix this in-round, but it has no real connection to a future extinction event" — never "P(ballot remedies | vote) > 0 while P(ballot prevents | vote) ≈ 0." If a technical term is standard debate vocabulary for the debater's level (per Section 3's calibration), it's fine; invented-sounding formal notation is not, at any level.
